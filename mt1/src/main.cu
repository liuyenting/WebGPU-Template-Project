...
__host__ void crossSum(int *data, int *outData, int size) {
  ...
  dim3 gridDim(
    ceil((float)size/TILE_WIDTH),
    ceil((float)size/TILE_WIDTH)
  );
  crossSumKernel<<<gridDim, blockDim>>>(
    data, data, outData,
...
// Compute C = A * B
__global__ void crossSumKernel(
...
int main(int argc, char **argv) {
  ...
  dim3 blockDim(TILE_WIDTH, TILE_WIDTH);
  dim3 gridDim(
    ceil((float)numCColumns/TILE_WIDTH),
    ceil((float)numCRows/TILE_WIDTH)
  );
